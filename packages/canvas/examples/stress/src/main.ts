...
function run(rowCount: number, tasksPerRow: number): void {
  ...
  // 3. Initial paint = renderer attaches and draws the canvas + sidebar.
  const initPaint = time(() => {
    ...
    e.use(createColumns({
      sidebarWidth: 260,
      treeColumn: 'id',
      persistWidths: 'ganttkit-stress-columns-canvas',
...
function renderMetrics(m: Metrics): void {
  ...
  document.getElementById('metrics')!.innerHTML = `
    <table>
      ...
      ${row('Chart DOM nodes (canvas)', num(m.domNodes))}
    </table>
