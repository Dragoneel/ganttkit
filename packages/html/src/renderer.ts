...
import { createPrimitive, h } from './dom'
...
/**
 * Options for the html renderer. Identical to every other base
 * renderer, so the three are drop-in swaps for one another.
 */
export type HtmlRendererOptions = RendererOptions

/**
 * Imperative HTML/DOM renderer for a GanttKit engine.
 *
 * The DOM-twin of `@ganttkit/svg`: it consumes the exact same renderer-
...
export class HtmlRenderer {
  ...
  private readonly opts: Required<Omit<HtmlRendererOptions, 'target' | 'theme'>>
  ...
  constructor(ctx: GanttContext, options: HtmlRendererOptions) {
    this.ctx = ctx
    this.opts = {
    ...
    const target = typeof options.target === 'string'
      ...
      : options.target
    if (!target)
      throw new Error('GanttKit/html: render target not found')
    this.root = target
    ...
    // don't reach into renderer DOM. The chart div is the scene-coordinate surface.
  ...
  private buildSkeleton(): void {
    ...
    // Positioned canvas the scene's primitives are absolutely placed into.
    this.chartEl = h('div', 'gantt__html')
    chart.appendChild(this.chartEl)
  ...
  private paint(scene: Scene): void {
    this.chartEl.style.width = `${scene.width}px`
    this.chartEl.style.height = `${scene.height}px`

    const frag = document.createDocumentFragment()
    for (const layer of scene.layers) {
      const g = h('div', `gantt__layer gantt__layer--${layer.name}`)
      for (const prim of layer.primitives)
        g.appendChild(createPrimitive(prim))
      frag.appendChild(g)
    }
    this.chartEl.replaceChildren(frag)
  }
  ...
  private wireEvents(): void {
    ...
    this.chartEl.addEventListener('mousedown', this.onTaskMouseDown)
    this.disposers.push(() => this.chartEl.removeEventListener('mousedown', this.onTaskMouseDown))

    this.chartEl.addEventListener('click', this.onTaskClick)
    this.disposers.push(() => this.chartEl.removeEventListener('click', this.onTaskClick))

    // Emit semantic hover events so plugins (e.g. tooltip) never hit-test the DOM.
    this.chartEl.addEventListener('mousemove', this.onTaskHover)
    this.disposers.push(() => this.chartEl.removeEventListener('mousemove', this.onTaskHover))
    this.chartEl.addEventListener('mouseleave', this.onTaskHoverEnd)
    this.disposers.push(() => this.chartEl.removeEventListener('mouseleave', this.onTaskHoverEnd))
