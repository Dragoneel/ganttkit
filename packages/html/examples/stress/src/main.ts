...
function run(rowCount: number, tasksPerRow: number): void {
  ...
  // 3. Initial paint = renderer attaches and renders the HTML chart + sidebar.
  const initPaint = time(() => {
    ...
    e.use(createColumns({
      sidebarWidth: 260,
      treeColumn: 'id',
      persistWidths: 'ganttkit-stress-columns-html',
  ...
  const domNodes = app.querySelector('.gantt__html')!.querySelectorAll('*').length
...
function renderMetrics(m: Metrics): void {
  ...
  document.getElementById('metrics')!.innerHTML = `
    <table>
      ...
      ${row('HTML DOM nodes (visible)', num(m.domNodes))}
    </table>
